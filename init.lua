...
-- Required for opencode.nvim to auto-reload edited buffers
vim.o.autoread = true
...
vim.keymap.set('n', '[d', vim.diagnostic.get_prev, { desc = 'Go to previous [D]agnoistic message' })
vim.keymap.set('n', ']d', vim.diagnostic.get_next, { desc = 'Go to next [D]agnoistic message' })
...
--
-- NOTE: Navigation now handled by vim-tmux-navigator plugin for seamless tmux integration
-- The plugin provides the same Ctrl+hjkl functionality plus tmux awareness
...
require('lazy').setup({
    ...
    opts = {
      ...
      sources = {
        ...
        providers = {
          ...
          -- Note: opencode source is automatically registered by opencode.nvim
          -- It provides completion for @this, @buffer, @diagnostics, etc. in opencode prompts
        },
      },
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      --  - gc{motion} - Toggle comment on motion (gcip to comment paragraph)
      --  - gc in visual mode - Toggle comment on selection
      require('mini.comment').setup()

      -- Better buffer deletion (preserves window layout)
      -- Use :lua MiniBufremove.delete() instead of :bdelete
      -- This prevents windows from closing when you delete a buffer
      require('mini.bufremove').setup()

      -- Move lines and selections with Alt+hjkl
      -- Examples:
      --  - Alt+h/l - Move current line or selection left/right
      --  - Alt+j/k - Move current line or selection down/up
      require('mini.move').setup()

      -- File explorer with Miller columns (shows parent/current/child in one view)
      -- Navigate and manipulate filesystem like a buffer
      -- Examples:
      --  - Press '-' to open file explorer
      --  - h/l to navigate up/down directories
      --  - Enter to open file
      --  - Edit text to create/delete/rename files, then press = to synchronize
      require('mini.files').setup {
        -- Customization of shown content
        content = {
          filter = nil, -- Predicate for which file system entries to show
          prefix = nil, -- What prefix to show to the left of file name
        ...
        mappings = {
          close = 'q',
          go_in = 'l', -- Enter to open file/directory
          go_in_plus = 'L',
          go_out = 'h',
          go_out_plus = 'H',
          mark_goto = "'",
      ...
      -- Keymap to open mini.files (vim-vinegar style)
      vim.keymap.set('n', '-', function()
        require('mini.files').open(vim.api.nvim_buf_get_name(0))
      end, { desc = 'Open mini.files (directory of current file)' })

      -- Optional: Open mini.files at current working directory
      vim.keymap.set('n', '<leader>-', function()
